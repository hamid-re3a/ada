...
class OrderController extends AuthController
{
...

    {
        ...
        $rules = array(
            ...
            'date' => array('required'),
            'time_slot' => array('required','number','max:20'),
        ...
        $obj = Order::where('date',$request->date)->where('time_slot',$request->time_slot)->first();
        if(isset($obj) && isset($obj->id)){
            return $this->respondBadRequest("این زمان قبلا رزرو شده است");
        }
        $input = $request->only('phone_number', 'gender', 'service_id','date','time_slot');
        $input['status'] = "paid";
...
//            $order->save();
            ...
            $message = " درخواست شما با کد پیگیری $order->id با موفقیت ثبت شد ";
            $this->sendSMS($user->phone_number,$message );

            return $this->respondSuccessfully('عملیات با موفقیت انجام شد');
//        } else {
//            $order->delete();
